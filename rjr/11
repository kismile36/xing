...
local Fluent = loadstring(game:HttpGet("https://pastefy.app/XeHanzcE/raw"))()
...
local HalloweenToggle = Tabs.Halloween:AddToggle("HalloweenToggle", {
Title = "自动不请自来",
Description = "",
Default = false
})
...
local DoorBySea = {
World1 = {
    CFrame.new(-1016.05951,11.1554155,1540.78235,-0.259074688,0,0.965857327,0,1,0,-0.965857327,0,-0.259074688),
    CFrame.new(-1089.28284,10.6633625,1604.43518,0.866141319,0,-0.499799222,0,1,0,0.499799222,0,0.866141319),
    CFrame.new(-1090.78882,10.5735903,1694.84729,0.965862036,0,0.259057134,0,1,0,-0.259057134,0,0.965862036),
    CFrame.new(-665.780701,9.71188927,1701.52112,-0.99181968,5.254558036e-08,-0.127646819,5.13036298e-08,1,1.30173978e-08,0.127646819,6.36216635e-09,-0.99181968),
    CFrame.new(-571.03418,9.71188927,1553.104,0.238623217,8.71156587e-08,-0.971112251,2.74919216e-08,1,9.64624505e-08,0.971112251,-4.97159226e-08,0.238623217),
    CFrame.new(-615.743652,9.68747139,1479.30908,0.815481067,0.00558292679,-0.578756809,0.000366810855,0.999948263,0.0101627586,0.578783572,-0.00849983189,0.8154369),
    CFrame.new(722.841309,7.79379559,1444.29028,0.977708578,-4.91986221e-08,-0.209966481,3.68222004e-08,1,-6.2854042e-08,0.209966481,5.37215108e-08,0.977708578)
},
World2 = {
    CFrame.new(-512.035645, 74.7451935, 156.465195, 0, 0, 1, 0, 1, -0, -1, 0, 0),
    CFrame.new(-420.992615, 74.4821396, 156.168015, 0, 0, 1, 0, 1, -0, -1, 0, 0),
    CFrame.new(-371.423737, 74.7515182, 154.181808, 0.719358087, 0, 0.694639385, 0, 1, 0, -0.694639385, 0, 0.719358087),
    CFrame.new(-637.225525, 74.6199265, 206.692734, -0.999848366, 0, -0.017436387, 0, 1, 0, 0.017436387, 0, -0.999848366),
    CFrame.new(-620.019348, 74.4223099, 103.196877, -0.707134247, 0.00017884915, -0.707079291, -1.93455853e-05, 0.99999994, 0.000272287783, 0.707079291, 0.000206222874, -0.707134247)
},
World3 = {
    CFrame.new(-12257.124,334.613007,-7417.19434,-0.996191859,0,-0.0871884301,0,1,0,0.0871884301,0,-0.996191859),
    CFrame.new(-12179.9883,335.293854,-7568.4043,0.984812498,0,0.173621148,0,1,0,-0.173621148,0,0.984812498),
    CFrame.new(-12307.4014,335.27951,-7572.65381,-0.173624277,0,-0.984811902,0,1,0,0.984811902,0,-0.173624277),
    CFrame.new(-12461.3984, 334.636444, -7703.56934, 0.707134247, 0, 0.707079291, 0, 1, 0, -0.707079291, 0, 0.707134247),
    CFrame.new(-12603.7588, 335.348114, -7739.28662, -0.939700961, 0, 0.341998369, 0, 1, 0, -0.341998369, 0, -0.939700961),
    CFrame.new(-12533.2305, 334.66803, -7269.23242, -0.166172266, 0, -0.98609668, 0, 1, 0, 0.98609668, 0, -0.166172266)
}
}

DoorPositions = World1 and DoorBySea.World1 or World2 and DoorBySea.World2 or World3 and DoorBySea.World3 or {}

local function FindNearestPrompt(position)
local nearest, dist = nil, 10
for _, v in pairs(workspace:GetDescendants()) do
    if v:IsA("ProximityPrompt") then
        local parent = v.Parent
        local root = parent:FindFirstChild("HumanoidRootPart") or parent:FindFirstChild("Head") or parent:FindFirstChild("Torso") or parent
        if root then
...
HalloweenToggle:OnChanged(function(value)
_G.HalloweenToggle = value
end)

spawn(function()
while task.wait(0.1) do
    if _G.HalloweenToggle then
        for i = 1, #DoorPositions do
            if not _G.HalloweenToggle then
                break
            end

            local cf = DoorPositions[i]
            ...
            -- Try to find and trigger the prompt
            local prompt = FindNearestPrompt(cf.Position)
            if prompt then
                pcall(function()
                    fireproximityprompt(prompt)
                    task.wait(knockDelay)
                    fireproximityprompt(prompt)
                end)
            end
            ...
            pcall(function()
                replicated.Remotes.HalloweenEvent:FireServer("AcceptReward")
                replicated.Remotes.HalloweenEvent:FireServer("Parlus")
                replicated.Remotes.HalloweenEvent:FireServer("ignore")
                local rf = replicated:FindFirstChild("{6306b822-ed5d-426c-b0c1-5cdb809cc7f5}")
                if rf and rf:FindFirstChild("RemoteFunction") then
                    rf.RemoteFunction:InvokeServer("ignore")
                end
            end)
            task.wait(trickDelay)
        end
    end
end
end)

local HalloweenDoorEnemies = Tabs.Halloween:AddToggle("AutoDefeatDoorEnemies", {
Title = "自动击败门口敌人",
Description = "",
Default = false
})

local HalloweenEnemies = {
"Cursed Skeleton",
"Vampire",
"Gorilla",
"Oni Soldier",
...
HalloweenDoorEnemies:OnChanged(function(value)
_G.AutoDefeatHalloweenDoorEnemie = value
end)

spawn(function()
while task.wait(0.5) do
    if _G.AutoDefeatHalloweenDoorEnemie then
        local success, err = pcall(function()
            for i, v in next, game.Workspace.Enemies:GetChildren() do
                if v:IsA("Model")
                    and v:FindFirstChild("Humanoid")
                    and v.Humanoid.Health > 0
                    and table.find(HalloweenEnemies, v.Name) then
                    
                    repeat
                        task.wait()
                        Attack.Kill(v, _G.AutoDefeatHalloweenDoorEnemie)
                    until not _G.AutoDefeatHalloweenDoorEnemie or not v.Parent or v.Humanoid.Health <= 0
                end
            end
        end)

        if not success then
            warn("Auto Defeat Halloween Door Enemie Error:", err)
            task.wait(1)
        end
    end
end
...
local AutoWerewolf = Tabs.Halloween:AddToggle("AutoWerewolf", {
Title = "自动击杀狼人",
Description = "",
Default = false
})

AutoWerewolf:OnChanged(function(value)
_G.Werewolf = value
end)
    ...
    if _G.Werewolf then
        local success, err = pcall(function()
            for i, v in pairs(game:GetService("Workspace").Enemies:GetChildren()) do
                if v:IsA("Model") and v.Name == "Unbound Werewolf" and v:FindFirstChild("Humanoid") and v:FindFirstChild("HumanoidRootPart") then
                    if v.Humanoid.Health > 0 then
                        repeat
                            task.wait()
                            Attack.Kill(v, _G.Werewolf)
                        until not _G.Werewolf or not v.Parent or v.Humanoid.Health <= 0
                    end
                end
            end
        end)
        ...
        if not success then
            warn("Werewolf AutoKill Error:", err)
            task.wait(1)
        end
    end
end
...
Tabs.Discord:AddButton({
    Title = "加入我的 Discord 服务器",
    Description = "",
    Callback = function()
        setclipboard(tostring("https://discord.gg/xYmKM425Fu"))
    end
})
local ReplicatedStorage = game:GetService("ReplicatedStorage")
local Players = game:GetService("Players")
if World3 then
    Tabs.Main:AddSection("自动刷级")
    local TeleportToggle = Tabs.Main:AddToggle("tpSubmarineWorker", {
        Title = "传送到潜艇工人（2600级以上，仅第三海域）",
        Description = "传送到潜艇工人（Level >= 2600，仅第三海域）",
        Default = false,
...
if World3 then
Tabs.Main:AddSection("天空暴君 Boss")
...
Tabs.Settings:AddSection("设置 / 配置")
local _Weapon = {
    "Melee",
    "Sword",
    "Blox Fruit",
...
local Initialize = Tabs.Settings:AddToggle("Initialize", {
    Title = "初始化攻击 [M1/近战/刀剑]",
    Description = "[ 不支持 Gas 的 M1 ]",
    Default = true
})
...
local Q = Tabs.Quests:AddToggle("Q", {
    Title = "自动修罗 双诅咒刀",
    Description = "开启自动修罗 双诅咒刀，自动完成修罗任务",
    Default = false
})
Q:OnChanged(function(Value)
    _G.CDK_TS = Value
...
Tabs.Mirage:AddSection("试炼任务 / 其他 V4")
...
spawn(function()
    ...
    Tabs.Mirage:AddSection("只有海岛3")
end
if World3 then
Tabs.Drago:AddSection("道场任务 & 龙道场岛")
local DojoQ = Tabs.Drago:AddToggle("DojoQ", {
    Title = "自动道场训练师",
    Description = "开启自动道场训练师，自动完成道场任务",
    Default = false
})
...
local BlazeEM = Tabs.Drago:AddToggle("BlazeEM", {
    Title = "自动龙猎人",
    Description = "开启自动龙猎人，自动合成龙焰燃料，自动收集龙焰燃料",
    Default = false
})
...
Toggle = Tabs.Drago:AddToggle("Toggle", {
    Title = "自动龙族 (V1)",
    Description = "开启后自动完成任务1、自动史前事件并收集龙蛋",
    Default = false
})
Toggle:OnChanged(function(Value)
    _G.DragoV1 = Value
...
local fireflower = Tabs.Drago:AddToggle("fireflower", {
    Title = "自动龙族 (V2)",
    Description = "开启自动击杀森林海盗，自动收集火焰花",
    Default = false
})
...
Toggle = Tabs.Drago:AddToggle("Toggle", {
    Title = "自动龙族 (V3)",
    Description = "开启自动击杀恐怖鲨鱼，自动完成海上事件",
    Default = false
})
Toggle:OnChanged(function(Value)
    _G.DragoV3 = Value
...
Toggle = Tabs.Drago:AddToggle("Toggle", {
    Title = "自动龙族遗物试炼",
    Description = "开启自动龙族遗物试炼，自动完成龙族遗物试炼",
    Default = false
})
Toggle:OnChanged(function(Value)
    _G.Relic123 = Value
...
Tabs.Drago:AddSection("只有海岛3")
end
if World3 then
Tabs.Prehistoric:AddSection("火山磁铁")
Toggle = Tabs.Prehistoric:AddToggle("Toggle", {
    Title = "自动合成火山磁铁",
    Description = "开启自动合成火山磁铁，自动合成火山磁铁，当拥有1个火山磁铁时停止",
    Default = false
})
Toggle:OnChanged(function(Value)
    _G.CraftVM = Value
...
Tabs.Prehistoric:AddSection("史前岛屿")
...
local Vocan = Tabs.Prehistoric:AddToggle("Vocan", {
    Title = "自动推进史前事件",
    Description = "开启自动修复火山，自动击杀火山幽灵，自动移除火山",
    Default = false
})
Vocan:OnChanged(function(Value)
...
Toggle = Tabs.Prehistoric:AddToggle("Toggle", {
    Title = "完成火山后自动重置",
    Description = "完成火山后自动重置，当没有收集恐龙骨头时重置",
    Default = false
})
Toggle:OnChanged(function(Value)
    _G.ResetPH = Value
...
spawn(function()
    ...
    Tabs.Prehistoric:AddSection("只有海岛3")
    end
if World3 then
Tabs.SeaEvent:AddSection("海上事件 & 自动航行")
local ListSeaBoat = {
    "Guardian",
    "PirateGrandBrigade",
    "MarineGrandBrigade",
...
local Q = Tabs.SeaEvent:AddToggle("Q", {
    Title = "自动传送至冰冻维度",
    Description = "开启自动传送至冰冻维度，自动传送至冰冻维度",
    Default = false
})
Q:OnChanged(function(Value)
    _G.FrozenTP = Value
...
Tabs.SeaEvent:AddSection("实体海上事件")
...
Tabs.SeaEvent:AddSection("狐狸岛 & 事件")
...
spawn(function()
    while wait(.2) do
        if workspace.Map:FindFirstChild("KitsuneIsland") or workspace._WorldOrigin.Locations:FindFirstChild("Kitsune Island") then
            Check_Kitsu:SetDesc(" 狐狸岛 : 存在")
        else
            Check_Kitsu:SetDesc(" 狐狸岛 : 未发现")
        end
    end
end)
local Q = Tabs.SeaEvent:AddToggle("Q", {
    Title = "自动寻找狐狸岛",
    Description = "开启自动寻找狐狸岛，自动寻找狐狸岛",
    Default = false
})
Q:OnChanged(function(Value)
...
Tabs.SeaEvent:AddButton({
    ...
    Tabs.SeaEvent:AddSection("只有海岛3")
    end

Tabs.Raids:AddSection("副本事件 & 自动团本")
...
Tabs.Combat:AddSection("本地玩家设置 & 杂项")
Q = Tabs.Combat:AddToggle("Q", {
    Title = "实例化 貂族 V3【无限】",
    Description = "开启无限貂族 V3",
    Default = false
})
Q:OnChanged(function(Value)
    InfAblities = Value
...
Tabs.Combat:AddSection("自瞄设置")

Q = Tabs.Combat:AddToggle("Q", {
    Title = "忽略同队",
    Description = "开启忽略同队，自瞄不会瞄准同队玩家",
    Default = false
})
Q:OnChanged(function(Value)
    _G.NoAimTeam = Value
...
Tabs.Combat:AddSection("Esp物品 & 实体 & 岛屿")
            ...
            if EspEventIsland then
                if (v.Name == "Mirage Island" or v.Name == "Prehistoric Island" or v.Name == "Kitsune Island") then
                    if not v:FindFirstChild("NameEsp") then
                        ...
                        local cname = v.Name
                        if v.Name == "Mirage Island" then cname = "蜃景岛" end
                        if v.Name == "Prehistoric Island" then cname = "史前岛" end
                        if v.Name == "Kitsune Island" then cname = "狐狸岛" end
                        v.NameEsp.TextLabel.Text = cname .. "   \n" .. round((plr.Character.Head.Position - v.Position).Magnitude / 3) .. " M";
                    end
                end
            ...
            if v.Name == "Barista Cousin" then
                ...
                elseif workspace:FindFirstChild("Gay") then
                    if not Gay:FindFirstChild("NameEsp") then
                        ...
                        Gay["NameEsp"].TextLabel.Text = ("Cousin 染色商" .. "   \n" .. round((plr.Character.Head.Position - v.HumanoidRootPart.Position).Magnitude / 3) .. " M")
                    end
                end
            end
        end
            ...
            if v.Name == "Legendary Sword Dealer " then
                ...
                elseif workspace:FindFirstChild("Lgd") then
                    if not Lgd:FindFirstChild("NameEsp") then
                        ...
                        Lgd["NameEsp"].TextLabel.Text = ("传说之剑商人" .. "   \n" .. round((plr.Character.Head.Position - v.HumanoidRootPart.Position).Magnitude / 3) .. " M")
                    end
                end
            end
        end
...
Tabs.Travel:AddSection("旅行 - NPCs")
for _, v in pairs(replicated.NPCs:GetChildren()) do
...
UnPortal = Tabs.Misc:AddToggle("UnPortal", {
    Title = "解锁所有传送门",
    Description = "解锁所有传送门，未击败 rip_indra 时解锁",
    Default = false
})
